...
vector<vector<int>> get_random_samples(const vector<vector<int>> &samples,
    ...
    vector<int> linii;  // vector de indecsi
    for (int i = 0; i < samples.size(); i++) {
        linii.push_back(i);
    }
    // amestecarea valorilor in vector
    std::random_shuffle(linii.begin(), linii.end());
    for (int i = 0; i < samples.size()-num_to_return; i++){
        linii.pop_back();
    }
    // crearea matricei de samples random
    for (int i = 0; i < linii.size(); i++) {
        ret.push_back(samples[linii[i]]);
    }
    return ret;
}
